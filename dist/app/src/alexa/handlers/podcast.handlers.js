...
module.exports = Alexa.CreateStateHandler(StatesConst.PODCAST, {
  ReadPodcasts: function() {
    ...
    this.emit(
      ...
      this.t(
        "CHOOSE_PODCAST",
        audioTitles.slice(0, numberOfPodcasts).toString()
      ),
      this.t(
        "CHOOSE_PODCAST",
        audioTitles.slice(0, numberOfPodcasts).toString()
  ...
  ReadMorePodcasts: function() {
    ...
    this.emit(
      ...
      this.t(
        choosenSpeachOutput,
        audioTitles.slice(start, LastReadPodcast).toString()
      ),
      this.t(
        choosenSpeachOutput,
        audioTitles.slice(start, LastReadPodcast).toString()
...
var controller = (function() {
  return {
    play: function() {
      ...
      var token = String(
        this.attributes["playOrder"][this.attributes["index"]]
      );
      var playBehavior = "REPLACE_ALL";
      var podcast =
        podcastData[this.attributes["playOrder"][this.attributes["index"]]];
      ...
      this.response
        .speak(this.t("PODCAST_HELP_EXIT"))
        .audioPlayerPlay(
          playBehavior,
          podcast.url,
          token,
...
function shuffleOrder(callback) {
  ...
  var array = Array.apply(null, { length: podcastData.length }).map(
    Number.call,
    Number
  );
